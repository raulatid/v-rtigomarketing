...
describe('the deployment stages', () => {
  it('are all zero at rest and all one when deployed', () => {
    expect(deploymentFrom(0)).toEqual({
      activation: 0,
      deploy: 0,
      resolve: 0,
      fieldAspect: 1,
    })
    expect(deploymentFrom(1)).toEqual({
      activation: 1,
      deploy: 1,
      resolve: 1,
      fieldAspect: 2,
  ...
  it('read in order: activation leads, deployment follows, resolve lands last', () => {
    // At the end of activation the field has barely begun opening and the logo
    // has not started; at the end of the opening the logo is still
    // arriving. The ranges overlap on purpose — the stages must not read as
    // three separate animations.
    const early = deploymentFrom(DEPLOYMENT_STAGES.activation[1])
    expect(early.activation).toBe(1)
    expect(early.deploy).toBeLessThan(0.05)
    expect(early.resolve).toBe(0)

    const mid = deploymentFrom(DEPLOYMENT_STAGES.deploy[1])
  ...
  it('is monotonic and continuous, so a reversal simply runs it backwards', () => {
    ...
    // a jump, not a slope.
    const MAX_STEP = 0.04
    let previous = deploymentFrom(0)
    for (let i = 1; i <= 200; i += 1) {
      const next = deploymentFrom(i / 200)
      for (const key of ['activation', 'deploy', 'resolve', 'fieldAspect'] as const) {
  ...
  it('derives the field aspect from the deploy stage, and from nothing else', () => {
    // The artwork field widens exactly as the deployment opens, so the lockup
    // can never be contain-fitted into a field the light has not opened yet.
    const d = deploymentFrom(0.5)
    expect(d.fieldAspect).toBeCloseTo(1 + d.deploy, 10)
  })

  it('clamps out-of-range and non-finite input', () => {
    expect(deploymentFrom(-1)).toEqual(deploymentFrom(0))
    expect(deploymentFrom(2)).toEqual(deploymentFrom(1))
    expect(deploymentFrom(Number.NaN)).toEqual(deploymentFrom(0))
  })
})
