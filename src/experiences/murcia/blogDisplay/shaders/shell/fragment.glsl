...
// five-pixel rim reads as a prop. The quality has to come from how the edge catches
// light, not from motion.
//
// `display.frag` has since adopted the same rule and lost its own `uTime`, so the
// whole display object is now static by construction.
//
// ## The rim is hardware, not a hologram
//
// It was tuned as one: a teal `uRimColor` picked to sit between the face's halo and
// the beams' cyan, with a strong "lit from below" bias. That has been reversed —
// neutral silver-grey, less base, less bias, slightly MORE grazing response. The
// mechanism below is unchanged; only what it is a picture of has changed. A machined
// aluminium edge catching room light does everything the teal was pretending to,
// and it survives being looked at closely, which the teal did not.
