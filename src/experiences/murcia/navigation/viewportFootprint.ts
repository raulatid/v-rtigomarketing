...
/**
 * Reduces the navigable bounds so that the CAMERA ITSELF cannot leave them.
 *
 *   effective = configured ∩ ((configured − cameraOffset) inset by safetyMargin)
 *
 * The camera sits at `focus + offset`, and that offset is a function of yaw,
 * pitch and distance ALONE — never of the focus. So "the eye is inside R" is
 * exactly "the focus is inside R shifted by −offset", which is a rectangle, and
 * the whole rule reduces to one more intersection in the same pipeline that
 * already insets by the viewport footprint.
 *
 * The shift is ONE-SIDED, and that is what makes the rule affordable. The camera
 * is on one side of the focus, not on all four, so the surviving width is
 * `width − |offsetX|`, not `width − 2·|offsetX|`. On Murcia's 352-unit plate at
 * the shipped pose that is the difference between ~172 units of pan and none at
 * all (DECISIONS §39).
 *
 * `safetyMargin` is not decoration. `DragPanController.setBounds` deliberately
 * re-clamps only the *target* and lets the smoothing draw the rendered focus in,
 * so a yaw held against the edge trails its own bounds by a frame or two. The
 * margin absorbs that trail, which is the same job it does for the footprint
 * term above.
 */
export function computeStationLimitedBounds(
  configured: BoundsRect,
  offsetX: number,
  offsetZ: number,
  safetyMargin: number,
): BoundsRect {
  const shifted: BoundsRect = {
    minX: configured.minX - offsetX + safetyMargin,
    maxX: configured.maxX - offsetX - safetyMargin,
    minZ: configured.minZ - offsetZ + safetyMargin,
    maxZ: configured.maxZ - offsetZ - safetyMargin,
  };

  return collapseIfInverted(intersectRect(configured, shifted));
}
