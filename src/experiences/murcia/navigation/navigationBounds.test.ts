...
// `describe('resistToRect')` was here — 114 lines covering the exponential soft
// band of DECISIONS §40: that the ramp cannot overshoot, that it composes
// exactly under coalesced pointer events, and that leaving the band is 1:1.
//
// The band went with §39 and §40 in the camera-navigation port. There is one
// rectangle now and the clamp against it is hard, because the rectangle is
// itself grown past the authored plate — the softness bought pan range, and the
// grown rect gives it outright.
