...
Supersedes nothing — this is a **delta** against `production-readiness-vercel.md` (2026-08-07),
...
| **SEC-1** | P1 | `?model=` same-origin guard is bypassable with a backslash. Found by the concurrent security audit (`security-wordpress-api.md`), gated behind `DEBUG_TOOLS_ENABLED`, so production is unaffected but Preview is not. **Owned by that audit, not restated here.** |
...
| ☑ | Security review completed | this pass + `security-wordpress-api.md` |
